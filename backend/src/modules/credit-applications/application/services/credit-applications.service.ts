...
export class CreditApplicationsService {
  ...
  async update(
    ...
    if (application.status !== 'DRAFT') {
      throw new BadRequestException(
        'Solo se puede editar una solicitud en estado DRAFT',
      );
    }
    ...
    if (command.dataAuthorized !== undefined)
  ...
  async simulateOffer(id: string): Promise<SimulationResult> {
    ...
    if (application.status !== 'DRAFT') {
      throw new BadRequestException(
        'La simulación solo aplica a solicitudes en estado DRAFT',
      );
    }

    if (application.term > 120) {
      throw new ServiceUnavailableException(
        'Error técnico temporal en la simulación. Intente más tarde.',
      );
    }
    ...
    if (
      ...
      return {
        status: 'not-viable',
        message: 'La solicitud no es viable con los datos actuales',
      };
    }
  ...
  async abandon(
    id: string,
    command: AbandonApplicationCommand,
  ): Promise<CreditApplication> {
    const application = await this.getById(id);
    if (
      ...
      throw new BadRequestException(
        'No se puede abandonar una solicitud finalizada',
      );
    }
