...
- **Respuestas de simulación en inglés:** el payload de `SIMULATED` y la respuesta del endpoint usan `status: 'approved' / 'not-viable'` y mensajes técnicos. El frontend las traduce, pero un API pública debería soportar `Accept-Language` o devolver códigos neutralmente localizables.
